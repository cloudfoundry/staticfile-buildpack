...
describe 'detect script' do
  context 'there is a Staticfile' do
    specify do
      stdout, stderr, status = Open3.capture3('bin/detect spec/fixtures/staticfile_app')
  ...
  context 'there is no Staticfile' do
    specify do
      stdout, stderr, status = Open3.capture3('bin/detect spec/fixtures/non_staticfile_app')
